...
/**
 * Upgrade the database to version 2004.
 *
 * @since 2.0.0
 *
 ...
 * @uses genesis_get_option()       Get theme setting value.
 */
function genesis_upgrade_2004() {

	//* Update Settings
	_genesis_update_settings( array(
		'theme_version' => '2.0.0-RC1',
		'db_version'    => '2004',
	) );
...
function genesis_upgrade() {
	...
	###########################
	# UPDATE DB TO VERSION 2004
	###########################

	if ( genesis_get_option( 'db_version', null, false ) < '2004' )
		genesis_upgrade_2004();
...
function genesis_update_nag() {
	...
	printf(
	__( "Genesis %1$s is available. %2$sCheck out what's new%3$s or %4$supdate now%3$s.", 'genesis' ),
		esc_html( $genesis_update['new_version'] ),
		'<a href="' . esc_url( $genesis_update['changelog_url'] ) . '" class="thickbox thickbox-preview">',
		'</a>',
		'<a href="' . wp_nonce_url( 'update.php?action=upgrade-theme&amp;theme=genesis', 'upgrade-theme_genesis' ) . '" class="genesis-js-confirm-upgrade">'
	);
	echo '</div>';
